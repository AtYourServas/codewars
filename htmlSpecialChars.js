...
function htmlSpecialChars(formData) {
    //Create Object of values to convert
    let removeChars = {
      '<': '&lt;',
      '>': '&gt;',
      '"': '&quot;',
    ...
    //create array to iterate thru
    let arr = formData.split('')
    //iterate thru each element of the array
    for (let i=0; i < arr.length; i++) {
        //check the element of the array against each character that should be replaced
      for (key in removeChars) {
          //replace if element matched
        if (arr[i] === key) {
          arr[i] = removeChars[key]
        }
      }
  }
  //return the array recombined as a string
      return arr.join('')
    }